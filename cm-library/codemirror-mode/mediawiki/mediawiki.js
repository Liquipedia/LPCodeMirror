	...
	CodeMirror.defineMode( 'mediawiki', function( config /* , parserConfig */ ) {
		...
		var mwConfig = config.mwConfig,
			...
			permittedHtmlTags = { b: true, bdi: true, del: true, i: true, ins: true,
				...
				tr: true, noinclude: true, includeonly: true, onlyinclude: true, translate: true, ref: true },
		...
		function eatTemplatePageName( haveAte ) {
			return function( stream, state ) {
				...
				if ( haveAte && stream.sol() ) {
					// Hack make first line comments in templates work
					if ( stream.sol() ) {
						state.nTemplate++;
						state.stack.push( eatTemplateArgument( haveAte ) );
					}
					// @todo error message
					state.nTemplate--;
					state.tokenize = state.stack.pop();
